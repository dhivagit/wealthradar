...
function ForgotPasswordView({ onBack }) {
  ...
  const handleSubmit = async () => {
    ...
    if (!serviceId || !templateId || !publicKey) {
      setLoading(false)
      setError('Email service not configured. Add VITE_EMAILJS_SERVICE_ID, VITE_EMAILJS_TEMPLATE_ID and VITE_EMAILJS_PUBLIC_KEY to your .env.local file.')
      return
    }

    try {
      const token     = Math.random().toString(36).slice(2) + Date.now().toString(36)
      const resetLink = window.location.origin + '?reset=' + token + '&email=' + encodeURIComponent(email.trim().toLowerCase())
      ...
      await window.emailjs.send(serviceId, templateId, {
        ...
        app_name:   'WealthRadar',
      })

      // Store token with 1-hour expiry in localStorage
      const expiry = Date.now() + 60 * 60 * 1000
      localStorage.setItem('wr_reset_' + token, JSON.stringify({
        email: email.trim().toLowerCase(), expiry,
      }))

      setSent(true)
    } catch {
      setError('Failed to send reset email. Check your EmailJS credentials in .env.local.')
    } finally {
      setLoading(false)
    }
  }
  ...
  if (sent) return (
    <div style={{ textAlign: 'center', padding: '12px 0' }}>
      ...
      <h3 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 22, color: '#1a1d2e', marginBottom: 10 }}>
        Reset link sent!
      </h3>
      <p style={{ fontSize: 14, color: '#6b7494', lineHeight: 1.6, marginBottom: 8 }}>
        We sent a reset link to <strong style={{ color: '#1a1d2e' }}>{email}</strong>
      </p>
      <p style={{ fontSize: 12, color: '#a0a8c0', marginBottom: 24 }}>
  ...
  return (
    <div>
      <button onClick={onBack} style={{ background: 'none', border: 'none', color: '#9098b8', cursor: 'pointer', fontSize: 13, marginBottom: 22, padding: 0, display: 'flex', alignItems: 'center', gap: 5, fontFamily: "'Outfit',sans-serif" }}>
        ← Back to Sign In
      </button>
      <h3 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 26, color: '#1a1d2e', marginBottom: 6 }}>
        Reset password
      </h3>
      <p style={{ fontSize: 13, color: '#8892b0', marginBottom: 24, lineHeight: 1.6 }}>
      ...
      {error && (
        <div style={{ fontSize: 12, color: '#dc2626', background: 'rgba(220,38,38,0.06)', border: '1px solid rgba(220,38,38,0.18)', borderRadius: 8, padding: '10px 12px', marginBottom: 16, lineHeight: 1.6 }}>
          ⚠ {error}
        </div>
      )}
      <button className="btn btn-gold"
        ...
        {loading
          ? <><span style={{ animation: 'spin 0.8s linear infinite', display: 'inline-block', marginRight: 4 }}>↻</span> Sending…</>
          : '📧  Send Reset Link'}
      </button>
    </div>
  )
