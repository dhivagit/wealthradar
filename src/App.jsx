...
// ── Inner shell (rendered when authenticated) ─────────────────────────────────
function Shell() {
  ...
  const currSymbol = settings?.currency
    ? ({ INR:'₹', USD:'$', EUR:'€', GBP:'£', JPY:'¥', CAD:'CA$', AUD:'A$', SGD:'S$', AED:'AED' })[settings.currency] || '₹'
    : '₹'
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#06070a' }}>
      ...
      <aside style={{
        ...
        <div style={{ padding: '20px 16px 16px', borderBottom: '1px solid #1a1f2e' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 34, height: 34, background: 'linear-gradient(135deg,#c8953a,#e8c060)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>⬡</div>
            <div>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
        ...
        <header style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <button className="btn btn-ghost btn-icon" style={{ fontSize: 18 }} onClick={() => setSidebarOpen(s => !s)}>☰</button>
            <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 19 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div onClick={() => setActiveTab('settings')} style={{ cursor: 'pointer' }}>
              {session?.picture
                ? <img src={session.picture} alt="" style={{ width: 32, height: 32, borderRadius: '50%', border: '2px solid #1a1f2e', display: 'block' }} />
                : <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg,#c8953a,#e8c060)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 600, color: '#06070a' }}>
      ...
      {notif && (
        <Notification msg={notif.msg} type={notif.type} onDone={() => setNotif(null)} />
      )}
    </div>
  )
}

// ── Root: wraps providers and decides auth vs app ─────────────────────────────
function AuthGate() {
  const { session } = useAuth()
